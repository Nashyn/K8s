...
public class ValidatorService {
    ...
    public static class ValidatorController {
        ...
        public ResponseEntity<Map<String, Object>> calculateAmount(@RequestBody Map<String, String> json_data) {
            ...
            try {
                boolean isHeaderValid = Files.lines(Paths.get(resource.getURI()))
                                        .findFirst()
                                        .map(line -> line.contains("product") && line.contains("amount"))
                                        .orElse(false);
                ...
                long sum = Files.lines(Paths.get(resource.getURI()))
                                .skip(1) 
                                .map(line -> line.split(","))
                                .filter(parts -> parts.length > 1 && parts[0].trim().equalsIgnoreCase(product))
                                .mapToLong(parts -> {
                                    try {
